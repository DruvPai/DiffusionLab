...
from diffusionlab.vector_fields import VectorField, VectorFieldType, convert_vector_field_type
...
def test_vector_field_conversion():
    ...
    x0_from_score = convert_vector_field_type(
        x, fx, alpha, sigma, alpha_prime, sigma_prime,
        VectorFieldType.SCORE, VectorFieldType.X0
    )
    score_back = convert_vector_field_type(
        x, x0_from_score, alpha, sigma, alpha_prime, sigma_prime,
    ...
    score_back = convert_vector_field_type(
        x, eps_from_score, alpha, sigma, alpha_prime, sigma_prime,
        VectorFieldType.EPS, VectorFieldType.SCORE
    )
    assert torch.allclose(fx, score_back)
    ...
    score_back = convert_vector_field_type(
        x, v_from_score, alpha, sigma, alpha_prime, sigma_prime,
        VectorFieldType.V, VectorFieldType.SCORE
    )
    assert torch.allclose(fx, score_back)
...
def test_vector_field_conversion_consistency():
    ...
    assert torch.allclose(v, alpha_prime.unsqueeze(-1) * x0 + sigma_prime.unsqueeze(-1) * eps) 