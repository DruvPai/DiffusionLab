...
from diffusionlab.samplers import FMSampler, VPSampler
...
def test_empirical_sampling(dummy_data):
    ...
    for i in range(N):
        ...
        assert any(torch.allclose(y[i], l) for l in all_labels)
...
def test_empirical_sampling_integration(sampler, dummy_data):
    ...
    N = 100
    X_sampled, y_sampled = EmpiricalDistribution.sample(N, {}, {"labeled_data": dummy_data})
...
def test_empirical_sampling_with_sampler(sampler, dummy_data):
    ...
    x0_est = VectorField(lambda x, t: EmpiricalDistribution.x0(x, t, sampler, {}, dist_hparams), vector_field_type=VectorFieldType.X0)
    eps_est = VectorField(lambda x, t: EmpiricalDistribution.eps(x, t, sampler, {}, dist_hparams), vector_field_type=VectorFieldType.EPS)
    v_est = VectorField(lambda x, t: EmpiricalDistribution.v(x, t, sampler, {}, dist_hparams), vector_field_type=VectorFieldType.V)
    score_est = VectorField(lambda x, t: EmpiricalDistribution.score(x, t, sampler, {}, dist_hparams), vector_field_type=VectorFieldType.SCORE)
