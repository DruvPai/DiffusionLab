...
class GMMDistribution(Distribution):
    def __init__(self, sampler: Sampler, means: torch.Tensor, covs: torch.Tensor, priors: torch.Tensor):
    ...
    def validate_params(cls, dist_params: Dict[str, Any]) -> None:
        assert "means" in dist_params and "covs" in dist_params and "priors" in dist_params
        means = dist_params["means"]
        covs = dist_params["covs"]
        priors = dist_params["priors"]
        assert len(means.shape) == 2
        K, D = means.shape
        assert len(covs.shape) == 3 and covs.shape[0] == K and covs.shape[1] == D and covs.shape[2] == D
        assert len(priors.shape) == 1 and priors.shape[0] == K
        assert means.device == covs.device == priors.device

        assert torch.allclose(torch.sum(priors), torch.tensor(1.0, device=priors.device))
    ...
    def stateless_x0(cls, sampler: Sampler, dist_params: Dict[str, Any], x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        covs_t = (alpha[:, None, None, None] ** 2) * covs [None, :, :, :] + (sigma[:, None, None, None] ** 2) * torch.eye(D, device=x.device)[None, None, :, :]  # (N, K, D, D)
        centered_x =  x[:, None, :] - alpha[:, None, None] * means[None, :, :]  # (N, K, D)
        ...
        w = torch.log(priors)[None, :] - 1/2 * logdets_covs_t - 1/2 * mahalanobis_dists  # (N, K)
        softmax_w = torch.softmax(w, dim=-1)  # (N, K)

        weighted_normalized_x = torch.sum(softmax_w[:, :, None] * covs_t_inv_centered_x, dim=-2)  # (N, D)
        x0_hat  = (1/alpha[:, None]) * (x - (sigma[:, None] ** 2) * weighted_normalized_x)# (N, D)
    ...
    def sample(self, N: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for k in range(K):
            ...
            X[idx] = torch.randn((X[idx].shape[0], D), device=device) @ cov_sqrt_k + means_k
        return X, y


class IsoHomoGMMDistribution(Distribution):
    def __init__(self, sampler: Sampler, means: torch.Tensor, var: torch.Tensor, priors: torch.Tensor):
    ...
    @classmethod
    def validate_params(cls, dist_params: Dict[str, Any]) -> None:
        assert "means" in dist_params and "var" in dist_params and "priors" in dist_params
        means = dist_params["means"]
        var = dist_params["var"]
        priors = dist_params["priors"]
        assert len(means.shape) == 2
    ...
    def stateless_x0(cls, sampler: Sampler, dist_params: Dict[str, Any], x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        weighted_normalized_x = torch.sum(softmax_w[:, :, None] * vars_t_inv_centered_x, dim=-2)  # (N, D)
        x0_hat  = (1/alpha[:, None]) * (x - (sigma[:, None] ** 2) * weighted_normalized_x)# (N, D)
    ...
    def sample(self, N: int) -> Tuple[torch.Tensor, torch.Tensor]:
        K, D = self.means.shape
        covs = torch.eye(D, device=self.var.device)[None, :, :].expand(K, -1, -1) * self.var
