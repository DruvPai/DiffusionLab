...
class DiffusionModel(LightningModule, VectorField):
    def __init__(
            self, net: nn.Module, sampler: Sampler, vector_field_type: VectorFieldType, 
        ...
        self.t_loss_weights_precomputed: torch.Tensor = self.t_loss_weights(self.sampler.schedule)
        self.t_loss_probs_precomputed: torch.Tensor = self.t_loss_probs(self.sampler.schedule)
        self.samplewise_loss: SamplewiseDiffusionLoss = SamplewiseDiffusionLoss(sampler, vector_field_type)

    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        return self.net(x, t)
    
    def configure_optimizers(self) -> Dict[Literal["optimizer", "lr_scheduler"], optim.Optimizer | optim.lr_scheduler.LRScheduler]:
        return {
    ...
    def aggregate_loss(self, x: torch.Tensor) -> torch.Tensor:
        t_idx = torch.multinomial(self.t_loss_probs_precomputed, self.N_noise_per_sample, replacement=True).to(x.device)
